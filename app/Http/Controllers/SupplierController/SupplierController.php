    ...
    public function ledger(Request $request)
{
    try {
        ...
        if ($startDate && $endDate) {
            $purchaseOrdersQuery->whereBetween('purchase_date', [$startDate, $endDate]);
        ...
        foreach ($purchaseOrders as $order) {
            ...
            $ledgerData->push([
                'date' => Carbon::parse($order->purchase_date)->format('Y-m-d'),
...
public function ledgerPDF(Request $request)
{
    try {
        ...
        return response()->json([
            'success' => true,
            'closing_balance' => $currencySymbol . ' ' . number_format($closingBalance, 2),
