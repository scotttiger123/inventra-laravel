		...
		$("#ProductCode_id").change(function () { alert();
					...
					success:function(dta)
						{	
							...
							$("#Cutt_Stock").html("");
								...
								$('#BalanceQuantity_id').val(dta[i].accum_qty);
								$('#BalanceQuantity_id_hidden').val(dta[i].accum_qty);
	...
	function 	IODataCall() {
		...
		$.ajax({
			...
			data:{
			 ...
			 ,'QuantityTo_Be_Cut_id'			:$("#QuantityTo_Be_Cut_id").val()
			 ,'AvailableQantity_id'				:$("#AvailableQantity_id").val()
			 ,'BalanceQuantity_id'				:$("#BalanceQuantity_id").val()
			 ,'Bill_Ref_id'						:$("#Bill_Ref_id").val()
			 ,'Salesman_id'						:$("#SalesMan").val()
			 ,'Cutting_Person_id'				:$("#Cutting_Person_id").val()
			 ,'LocationStore_id'				:$("#LocationStore_id").val()
			 ,'Cutting_From_Stock_Id_id'		:$("#Cutting_From_Stock_Id_id").val()
			 ,'display_location'		        :$("#display_location").val()
			 
			},
			dataType:'text',
			url:"PHP/InsertOutwardFromWarehouse.php",
			success:function(dta)
			{	
