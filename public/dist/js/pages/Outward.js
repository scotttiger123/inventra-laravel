/*this is new module old is outwardwarehouse.js */
...
function get_purchase_items(DSLValueTest,Extra){
			
			var lfckv = document.getElementById("letter_head").checked;
			var DSLValue = $("#order_id").val();
			myWindow = window.open("PHP/get_purchase_items.php?order_id="+DSLValue+"&^%^$@*^^%@1&&%%^*&*2345@="+Extra+"&checkbox="+lfckv, '', 'width=600,height=300,scrollbars=1','location=no','menubar=0');
			myWindow.focus();
...
$("#PostWindows").click(function () {
   ...
   $("#ProductCode_id").change(function(){
	  ...
	  var pname = $("#ProductCode_id").val(); 
	  var stock = findProductStock(pname);
      console.log("Stock:", stock);
      $('#avbl_stock').html(stock);
      
	  
});

function findProductStock(selectedProductId) {
                        var product = ProductStock.find(function(product) {
                        return product.product_name === selectedProductId;
                    });
   ...
   function getProductId() {
    ...
    var productName = $("#ProductCode_id").val();
    var productInfo = {};
  ...
  function IOData()
	{
        ...
        var PVal = $("#ProductCode_id").val();
		var Qty  = $('input[name="Qty"]').val();
        
    	    var stock = findProductStock(PVal) - Qty;//new stock getting 
            if (stock < 0) {
                alert('Stock is zero!');
                return;
            }
	...
	//	alert($("#Client").val());
	  ...
	  var dateString = document.getElementById('datetimepicker_dark1').value;
        ...
        */
		...
		if( parseFloat(input_value) >  parseFloat(stock_value) ){
			
			alert("missing qty");
			$('#QuantityTo_Be_Cut_id').focus();
			return
		}
     ...
     if ( $("#QuantityTo_Be_Cut_id").val() == "" || $("#QuantityTo_Be_Cut_id").val() < 1 ) { 
	     ...
	     if ( $("#LocationStore_id").val() == "" ) { 
	    
	            alert('Add locatin store id .');
                return false;
		...
		$.ajax({
			type:"POST",
			data:{'PVal':PVal,'Qty':$("#QuantityTo_Be_Cut_id").val()
			,'BalanceQuantity_id':$("#BalanceQuantity_id").val()
			,'Cutting_Person_id':$("#Cutting_Person_id").val()
			,'LocationStore_id':$("#LocationStore_id").val()
			,'date':$("#datetimepicker_dark1").val()
			,'OrderNo':$("#OrderNo").val()
			
			,'ProductName':$("#ProductCode_id").val()
			...
			,'inward_store':$("#inward_store").val()
			
			},
			dataType:'text',
			url:"PHP/insert_temp_outward_item.php",
			success:function(dta)
			{	
			    console.log(dta);
			    $("#inward_store").val("");
				...
				$("#InvResponse").load("PHP/outward_temp_load.php");
				//$("#ForPirnt").load("PHP/POSPrint.php");
	...
	window.onload = function() { // load data in temp table on page load 
		$("#InvResponse").load("PHP/purchase_temp_load.php");
				//$("#ForPirnt").load("PHP/POSPrint.php");
};
	...
	$("#CreateNewCustomer").click(function () {
		...
		$.ajax({
			...
			dataType:'text',
			url:"PHP/create_new_suppliers.php",
			success:function(dta)
			{	
...
$("#SavePrint").click(function () {
	...
	$("#spinner").show();
		$.ajax({
			...
			dataType:'text',
			url:"PHP/insert_outward_order.php",
			success:function(dta)
			{	
	...
	*/	
			function edit_invoice (){
					var myWindow = window.open("PHP/edit_purchase.php?order_id="+$("#order_id").val(), '', 'width=500,height=300,scrollbars=1','location=no','menubar=0');
					myWindow.focus();	
			} 			
	...
	function DelRecored(objRow)
	{
		...
		$.ajax({
			...
			data: { 'ID' : objRow.value,'Table' :'temp_outward_item' ,'Col' : 'order_item_id' },
			success:function(dta)
			{	
				$("#InvResponse").load("PHP/outward_temp_load.php");
				// $("#ForPirnt").load("PHP/POSPrint.php");  
